    ...
    <Link to={`/ads/${_id}`} className="text-decoration-none text-black">
      <div className="row g-0">
        <div className="col-md-4">
          {images && images[0] && images[0].url ? <img src={`${BACKEND_URL}${images && images[0] && images[0].url}`} className="img-fluid rounded-start" alt={title} /> : <img src={NO_IMAGE} className="img-fluid rounded-start" alt={title} />}
        </div>
...
const MyAds = ({ ads = [], status, onMyAds}) => {
  useEffect(() => {
    onMyAds()
  }, [])
  return (status === 'PENDING' || !status ? <Loader /> :
        ...
        {imgArray.map((img, index) =>
          index === 0 ?
            <div key={img._id} className="carousel-item active">
              <img className="d-block w-100" alt="picture" src={`${BACKEND_URL}${img.url}`} />
            </div> :
...
const AdCardDetailed = ({ ad: { title, images, price, description, phones, address } }) => {
  ...
  return (
    <div className='card' style={{width: 100+"%"} }>
      ...
      <SliderImageries imgArray={images}/>
      <div className="card-body">
        <h3 className="card-title">{title}</h3>
...
const CPageAds = connect(state => ({ ad: state.info?.goodById?.payload }), { onIdChange: actionGoodById })(PageAds);
