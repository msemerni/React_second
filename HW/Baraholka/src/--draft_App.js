...
const actionAddAd = (adTitle, adDescription, adPrice, adAddress, adImages) => {
  const queryPromise = gql(
      `mutation AddAd ($adTitle: String, $adDescription: String, $adPrice: Float, $adAddress: String, $adImages:[ImageInput]) {
        AdUpsert (ad: {title: $adTitle, description: $adDescription, price: $adPrice, address: $adAddress, images: $adImages})
        {_id title tags description price address images{url}}
      }`,
      { adTitle, adDescription, adPrice, adAddress, adImages })
...
const actionImgeriesUpload = async (acceptedFiles) => {
  const uploadImg = async (acceptedFiles) => {
    ...
    myFormData.append("photo", acceptedFiles); 
  ...
  let allPromises =  await Promise.all(acceptedFiles.map(img => uploadImg(img)))
...
const Dashboard  = () => {
  ...
  const [imagesIds, ChangeImagesIds] = useState([])
///////////////////////
  useEffect(() => {
  ...
  const {getRootProps, getInputProps} = useDropzone({
    ...
    onDrop: acceptedFiles => {
        ...
        //// ДИСПАТЧИТЬ ЭКШН который отправляет файлы на бэк
        console.log("ACC F:", acceptedFiles)
      // actionImgeriesUpload(acceptedFiles);
    }
  });
...
console.log('!!!imagesIds!!',imagesIds)
    ...
    return (
      <form className='mx-auto p-5 col-12 text-center bg-light'>
        ...
        <button type="submit" className="btn btn-outline-success" onClick={(e) => 
                                                              {store.dispatch(actionAddAd(title, description, +price, address, files));
                                                              e.preventDefault()
