...
//                         {onSearchAd: actionSearchAds})(SearchAd)
////////////////////
...
const actionChangeProfile = (myProfile) => 
  ...
  const gqlQuery = `mutation ChangeProfile($myProfile: UserInput) {
    UserUpsert(user: $myProfile) {
      _id, createdAt, login, nick, avatar {_id url}, phones, addresses
    }
  }`
...
const MyProfile = ({ myProfile: {_id, createdAt, login, nick, avatar, phones, addresses}, 
  ...
  useEffect(() => {
    // onChangeProfile(myProfile)
    console.log("my_id=>>>", _id);
...
{/* 
    <div className="col-md-4 p-2 mx-auto">
          {avatar && avatar.url ? 
          <img src={`${BACKEND_URL}${avatar && avatar.url}`} className="img-fluid rounded-circle" style={{height: 150 + "px" }} alt={"avatar.originalFileName"} /> 
          : <img src={NO_IMAGE_AVA} className="img-fluid rounded-circle" style={{height: 150 + "px" }} alt={"avatar.originalFileName"} />}
    </div> */}
    ...
    <div className="mb-2 row">
      ...
      <div className="col-sm-8">
        <input type="text" id="inputEmail" className="form-control" placeholder='*required'value={myLogin} required 
...
const CMyProfile = connect(state => ({myProfile: state.info?.userProfile?.payload, 
                                    ...
                                    //  onUpdateUserInfo: actionUser
                                    //  onUploadImg: actionImgeriesUpload
                                    })(MyProfile)


// const defIm = [{url: "images/404f0315900ab28438e382e1e64ea5c3", _id: "62fb93f1c1b7470e6a8930d0"}]

const PreviewAvatar = ({ img = [store.getState().info?.userProfile?.payload?.avatar], status, className }) => {
  useEffect(() => {
  }, [img])
  console.log("imageriesAVA+: ", img);
    // console.log("imageriesAVA[0]+: ", img[0]);
...
const Dashboard  = ({ newImg, onUpload, onNewAd}) => {
  // const _id = "62facbffc1b7470e6a8930a9";
  ...
  const addNewAd = () => {
    ...
    const newAd = {
    ...(_id ? {_id: _id} : {}),
    ...(imgArr.length ? {avatar: imgArr[0].url} : {}), ///  ко всем применить?
    title: title,
    ...
    <div className="row m-0">
      {
        ...
        // ads.map(ad => ad).sort((a, b) => a.title - b.title).map(ad => <AdsCategory ad={ad} key={ad._id} />)
      }
    </div>
  )
}
...
const AdCardDetailed = ({ ad: { title, images, price, description, phones, address } }) => {
  ...
  return (
    <div className='card' style={{width: 100+"%"}}>
      ...
      {/* {images?.length > 0 && <img className="card-img-top" src={`${BACKEND_URL}${images && images[0] && images[0].url}`}
        alt={title}></img>} */}
...
const Avatar = ({ avatar, status, className }) => {
  useEffect(() => {
    ...
    // console.log("++AVA_URL++: ", avatar.url);
  }, [avatar])
